...
class GaussianModel:
    ...
    def get_rotation(self):
        ...
        #mask = torch.zeros(self._rotation.shape).cuda()
        #mask[:,0] = 1
        #self._rotation = self._rotation * mask
        #self._rotation[:,3] += self._rotation[:,0]
    ...
    def create_from_pcd(self, pcd : BasicPointCloud, spatial_lr_scale : float, scale : float):
        self.spatial_lr_scale = spatial_lr_scale
        #fused_point_cloud = torch.tensor(np.asarray(pcd.points)).float().cuda()
        #print(np.random.rand((pcd.points.shape,3)))
        fused_point_cloud = torch.tensor(np.random.rand(pcd.points.shape[0],3)*2-1).float().cuda()
        fused_point_cloud = torch.tensor(pcd.points).float().cuda()
        #fused_color = RGB2SH(torch.tensor(np.asarray(pcd.colors)).float().cuda())
        fused_color = (torch.tensor(np.random.rand(pcd.points.shape[0],3)).float().cuda())
        fused_color = torch.tensor(pcd.colors).float().cuda()
        features = torch.zeros((fused_color.shape[0], 3, (self.max_sh_degree + 1) ** 2)).float().cuda()
        features[:, :3, 0 ] = fused_color
        features[:, 3:, 1:] = 0.0
        ...
        dist2 = torch.clamp_min(distCUDA2(torch.from_numpy(np.asarray(pcd.points)).float().cuda()), 0.0000001)
        scales = torch.log(torch.sqrt(dist2))[...,None].repeat(1, 3)
        #scales = torch.ones((fused_point_cloud.shape[0], 3), device="cuda") * scale
        #rots = torch.zeros((fused_point_cloud.shape[0], 4), device="cuda")
        #rots[:, 0] = 1
        # SMoE
        rots = torch.ones((fused_point_cloud.shape[0], 4), device="cuda") * scale
        #rots = torch.rand((fused_point_cloud.shape[0], 4), device="cuda") * scale
        # GS
        #rots = torch.rand((fused_point_cloud.shape[0], 4), device="cuda") * scale
        rots[:, 1] = 0
    ...
    def load_ply(self, path):
        ...
        print("Number of points after training {}".format(xyz.shape[0]))
    
    def load_multi_ply(self, paths):

        xyzs = torch.tensor([])
        features_dcs = torch.tensor([])
        features_extras = torch.tensor([])
        opacities = torch.tensor([])
        scaless = torch.tensor([])
        rotss = torch.tensor([])
        ...
        for path in paths:
            ...
            xyzs = torch.cat((xyzs,xyz),0)
            features_dcs = torch.cat((features_dcs,features_dc),0)
            features_extras = torch.cat((features_extras,features_extra),0)
            opacitiess = torch.cat((opacitiess,opacities),0)
            scaless = torch.cat((scaless,scales),0)
            rotss = torch.cat((rotss,rots),0)
        ...
        print("Number of points after training {}".format(self._xyz.shape[0]))
    ...
    def set_gaussian_indexed(self, rotation: torch.Tensor, indices: torch.Tensor):
